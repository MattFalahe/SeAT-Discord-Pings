 ...
 *
 * Future rollout: same pattern would slot cleanly into Matt's other
 * plugins (Mining Manager / Structure Manager / Corp Wallet Manager /
 * Buyback / HR Manager) by changing the constants here. The shape of
 * the returned array stays the same — Blade renderers can be copied.
 */
class VersionChecker
{
    /** Packagist v2 metadata URL for this package. */
    private const PACKAGIST_URL = 'https://repo.packagist.org/p2/mattfalahe/seat-discord-pings.json';

    /** Composer package key under packages.* in the Packagist response. */
    private const PACKAGE_KEY = 'mattfalahe/seat-discord-pings';

    /** Cache key for the fetched-latest result. */
    private const CACHE_KEY = 'discordpings:packagist_latest';
    ...
    public function getStatus(): array
    {
        ...
        // Dev branch installs (composer require ...:dev-dev, dev-main, etc.)
        ...
        if ($isDevBranch) {
            return [
                ...
                'latest'         => $latest,
                'status'         => 'dev_branch',
                'message'        => 'You are running a development branch (' . $current . '), not a tagged release. The "Latest release" pill above is the most recent stable Packagist tag — your branch may be ahead of or behind that depending on local commits. Switch to a tagged version (composer require mattfalahe/seat-discord-pings:^X.Y.Z) for a definitive "up to date" comparison.',
                'release_url'    => null,
            ];
        }
        ...
        if ($cmp < 0) {
            return [
                ...
                'release_url'    => 'https://github.com/MattFalahe/seat-discord-pings/releases/tag/' . $latest,
            ];
        }
     ...
     *   - 'config'   : fallback to discordpings.config.php (may be aspirational)
     *
    ...
    protected function resolveInstalledVersion(): array
    {
        ...
        return [(string) config('discordpings.version', '0.0.0'), 'config'];
    }

    /**
     * True if the version string looks like a Composer dev-branch ref
     * (e.g. 'dev-dev', 'dev-main', 'dev-feature/foo'). These can't be
     * version_compare'd meaningfully against semver tags.
     */
            ...
            try {
                $client = new Client([
                    ...
                    'headers'         => [
                        ...
                        'User-Agent' => 'SeAT-Broadcast/' . config('discordpings.version', 'unknown'),
                    ],
                ]);
                ...
                if (! is_array($data) || ! isset($data['packages'][self::PACKAGE_KEY]) || ! is_array($data['packages'][self::PACKAGE_KEY])) {
                    Log::warning('[DiscordPings] VersionChecker: Packagist response missing expected packages.' . self::PACKAGE_KEY . ' shape');
                    return null;
                }
                ...
                Log::warning('[DiscordPings] VersionChecker: failed to fetch latest version from Packagist: ' . $e->getMessage());
                return null;
            }
        });
    }
